...
use crate::api::analytics::models::{
    ...
    AdminRetentionResponse, AdminRiskDistributionResponse, AnomalyFlagItem,
    BehaviourProfileResponse, CohortDataPoint, ExportResponse, RiskBand,
};
...
pub async fn get_activity(
    ...
    let aggs = match state
        .repo
        .get_daily_aggregates(from.date_naive(), to.date_naive())
        .await
    {
        Ok(a) => a,
        Err(e) => return internal_err(&e.to_string()),
    ...
    let total_cngn: bigdecimal::BigDecimal =
        aggs.iter().map(|a| a.total_cngn_transferred.clone()).sum();
    let total_onramped: bigdecimal::BigDecimal =
        aggs.iter().map(|a| a.total_fiat_onramped.clone()).sum();
    let total_offramped: bigdecimal::BigDecimal =
        aggs.iter().map(|a| a.total_fiat_offramped.clone()).sum();
    let total_txs: i64 = aggs.iter().map(|a| a.total_tx_count).sum();
    let avg_size = if !aggs.is_empty() {
        aggs.iter()
            .map(|a| a.avg_tx_size.clone())
...
pub async fn get_retention(
    ...
    let (retained, churned) = match state
        .repo
...
pub async fn get_cohorts(
    ...
    let cohorts: Vec<CohortDataPoint> = rows
        .into_iter()
        .map(|(month, size, active)| {
            let retention_rate = if size > 0 {
...
pub async fn get_risk_distribution(State(state): State<Arc<AdminAnalyticsState>>) -> Response {
    ...
    let bands: Vec<RiskBand> = dist
        .into_iter()
        .map(|(band, count, min_s, max_s)| RiskBand {
            band,
            min_score: min_s,
            max_score: max_s,
            wallet_count: count,
    ...
    (
        StatusCode::OK,
        Json(AdminRiskDistributionResponse {
            bands,
            avg_risk_score: avg,
...
pub async fn get_anomalies(State(state): State<Arc<AdminAnalyticsState>>) -> Response {
    ...
    let items: Vec<AnomalyFlagItem> = rows
        .iter()
        .map(|r| AnomalyFlagItem {
            id: r.id,
