...
fn resolve_range(q: &AnalyticsQuery) -> (chrono::DateTime<Utc>, chrono::DateTime<Utc>) {
    ...
    match q.range.unwrap_or(TimeRange::Last30Days) {
        ...
        TimeRange::CurrentYear => {
            let start = now.with_month(1).and_then(|d| d.with_day(1)).unwrap_or(now);
            (start, now)
        }
        TimeRange::Custom => (
...
pub async fn get_summary(
    ...
    let prev_snaps = state
        .repo
        .get_snapshots(&wallet_id, period, prev_from, from)
        .await
        .unwrap_or_default();
    ...
    let delta_tx = prev.map(|p| {
        if p.total_tx_count > 0 {
            (current.total_tx_count as f64 - p.total_tx_count as f64) / p.total_tx_count as f64
                * 100.0
        } else {
            0.0
        }
...
pub async fn get_spending(
    ...
    let cats = match state
        .repo
        .get_spending_categories(&wallet_id, "monthly", from)
        .await
    {
        Ok(c) => c,
        Err(e) => return internal_err(&e.to_string()),
    ...
    let items: Vec<SpendingBreakdownItem> = cats
        .iter()
        .map(|c| SpendingBreakdownItem {
    ...
    (
        StatusCode::OK,
        Json(SpendingBreakdownResponse {
            wallet_address: wallet_id,
            period_start: from,
            period_end: _to,
            categories: items,
...
pub async fn get_trends(
    ...
    let points: Vec<TrendDataPoint> = snaps
        .iter()
        .map(|s| TrendDataPoint {
...
pub async fn get_counterparties(
    ...
    let items: Vec<CounterpartyItem> = rows
        .iter()
        .map(|r| CounterpartyItem {
...
pub async fn get_providers(
    ...
    let snaps = state
        .repo
        .get_snapshots(&wallet_id, "daily", from, to)
        .await
        .unwrap_or_default();

    let mut provider_map: std::collections::HashMap<String, (i64, f64)> =
    ...
    let items: Vec<ProviderUsageItem> = provider_map
        .into_iter()
        .map(|(provider, (count, _))| {
            ProviderUsageItem {
                provider,
                tx_count: count,
...
pub async fn get_insights(
    ...
    let items: Vec<InsightResponse> = rows
        .iter()
...
pub async fn get_insight_preferences(
    ...
    let prefs = state
        .repo
        .get_insight_preferences(&wallet_id)
        .await
        .unwrap_or(None);
    let (weekly, monthly) = prefs.unwrap_or((true, true));
    (
        StatusCode::OK,
        Json(InsightPreferencesResponse {
            wallet_address: wallet_id,
...
pub async fn update_insight_preferences(
    ...
    match state
        .repo
        .upsert_insight_preferences(&wallet_id, body.weekly_insights, body.monthly_insights)
        .await
    {
        Ok(_) => (
            StatusCode::OK,
...
pub async fn export_analytics(
    ...
    (
        ...
        Json(ExportResponse {
            ...
            message: format!(
                "Export {} queued. You will be notified when ready.",
                export_id
            ),
        }),
    )
