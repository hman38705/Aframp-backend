...
| 2 | Payment request generation | A merchant can generate a request for a specific amount | 🚧 **Not started** — no endpoint exists yet. This is the next major gap (see [§12](#12-roadmap)). |
| 3 | QR-based payment | A customer can scan a QR code to pay | 🚧 **Not started** — depends on #2 |
...
1. A customer sends a Stellar payment to the merchant's address (currently: manually, or via friendbot on testnet — no QR/request UI exists yet, see [§4](#4-goals--mvp-scope) row 2–3).
2. The background worker's next poll cycle (default every 60s, configurable) detects the payment via Horizon.
3. The payment is recorded, moved through `detected → verified → confirmed`, and the merchant's balance updates.
4. Merchant sees the payment in `GET /balance` and `GET /transactions`.

**Payment (deposit) flow — intended end state (not built)**
1. Merchant enters an amount in the app.
2. Aframp generates a payment request + QR code encoding it.
3. Customer scans with any Stellar-compatible wallet and pays.
4. Same detection/confirmation pipeline as above, but the merchant sees "Payment received" tied to the specific request, not just a raw balance change.
...
- **What's explicitly *not* built:** outbound Stellar transaction construction/signing, a settlement/sweep wallet, any real payout provider integration, QR/payment-request generation.
...
**✅ Verified working today:** merchant signup/login, universal auth enforcement, real Stellar wallet generation, encrypted key custody, real Stellar deposit detection (proven with an actual testnet transaction), balance ledger, transaction history, withdrawal request + ledger accounting.

**🚧 Genuine gaps:** payment request generation, QR-based payment, real payout provider (no money leaves the system), confirmation-depth threshold (currently immediate), settlement/sweep wallet, and one piece of vestigial dead code (`src/stellar/mod.rs`, from an abandoned memo-based design) still sitting in the tree as cleanup debt.
